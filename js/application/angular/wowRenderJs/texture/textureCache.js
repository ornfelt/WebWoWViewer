...
class Texture {
    ...
    loadFromMipmaps(mipmaps, textureFormat, hasAlpha) {
        ...
        // For demonstration, we won't do software decoding:
        // If the extension or format is missing, we either skip or do an uncompressed fallback.
        // Here we’ll just show a warning and skip.
        const canUseCompressed = (ext && textureGPUFormat);
        ...
        switch (textureFormat) {
            ...
            case "S3TC_RGBA_DXT3":
            case "S3TC_RGBA_DXT5":
                for (let k = 0; k < mipmaps.length; k++) {
                    if (canUseCompressed) {
                        // Use hardware-accelerated compressed textures
                        ...
                        console.warn(
                            `DXT fallback not available for ${textureFormat}, texture may not render.`
                        );
                        // Optionally, you could do an uncompressed fallback:
                        // gl.texImage2D(...some RGBA fallback...);
                    }
                }
                break;

            case "BGRA":
                // This was already uncompressed, so we can just pass it directly
                for (let k = 0; k < mipmaps.length; k++) {
                    gl.texImage2D(
                        gl.TEXTURE_2D,
                        k,
        ...
        if (anisFilterExt) {
            const maxAnisotropy = gl.getParameter(anisFilterExt.MAX_TEXTURE_MAX_ANISOTROPY_EXT);
            gl.texParameterf(gl.TEXTURE_2D, anisFilterExt.TEXTURE_MAX_ANISOTROPY_EXT, maxAnisotropy);
        }
        ...
        this.cache = cacheTemplate(
            ...
            (blpFile) => {
                ...
                textureObj.loadFromMipmaps(
                    blpFile.mipmaps,
                    blpFile.textureFormat,
                    blpFile.alphaChannelBitDepth > 0
                );
                textureObj.fileName = blpFile.fileName;
                return textureObj;
            }
